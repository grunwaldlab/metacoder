...
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(subset(data, name == "Firmicutes"),
       vertex_size = item_counts, vertex_label = name,
...
```{r}
set.seed(1)
plot(subset(data, taxon_ranks > 2),
       vertex_size = item_counts, vertex_label = name,
       vertex_color = item_counts, tree_label = name)
```
...
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(unite_ex_data_3, vertex_size = item_counts, vertex_label = name,
       vertex_color = item_counts, title = "All data"),
  plot(subset(subsampled, item_counts > 0, subtaxa = FALSE),
     vertex_size = item_counts, vertex_color = item_counts,
     vertex_label = item_counts, title = "Sampled"))
```
...
plot(subsetted, vertex_size = item_counts, vertex_label = name, 
     vertex_color = item_counts, overlap_avoidance = 0.5, tree_label = name)
```
...
set.seed(3)
plot(sampled, vertex_size = item_counts, vertex_label = name, 
     vertex_color = item_counts, overlap_avoidance = 0.5, tree_label = name)
```
...
```{r, fig.height=2.5}
set.seed(3)
plot(pcr, vertex_size = item_counts, vertex_label = name, 
     vertex_color = prop_amplified, vertex_color_range =  c("red", "cyan"),
...
## *In silico* PCR use case example: Differntial plot

```{r, fig.height=2.5}
subset(pcr, name == "Bacteria") %>%
  subset(count_amplified < item_counts | count_amp_2 < item_counts, subtaxa = F) %>%
  plot(vertex_size = item_counts, vertex_label = name,
