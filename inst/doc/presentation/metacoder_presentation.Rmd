...
contaminants <- extract_taxonomy(ids, key = c("item_id"),
                                 database = "ncbi")
```
...
```{r }
taxon_data(its1_ex_data, row_subset = 17:20) 
item_data(its1_ex_data, row_subset = 1:4) 
```
...
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(data, node_size = n_items, overlap_avoidance = 10),
  plot(data, node_size = n_items, overlap_avoidance = 0.1))
```
...
```{r}
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(data, node_size = n_items, node_color = n_items,
       node_color_range = c("#FFFFFF", "darkorange3", "#4e567d")),
  plot(data, node_size = n_items, node_color = "grey",
       edge_color = n_items))
```
...
```{r}
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(data, node_size = n_items, node_label = name,
       title = "Node labels"),
  plot(data, node_size = n_items, edge_label = name,
...
```{r}
set.seed(3)
plot(contaminants, node_size = n_items,
     node_color = n_items, node_label = name,
...
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(subset(data, name == "Firmicutes"),
       node_size = n_items, node_label = name,
       node_color = n_items),
...
plot(subset(data, taxon_levels > 2),
       node_size = n_items, node_label = name,
       node_color = n_items, tree_label = name)
```
...
gridExtra::grid.arrange(ncol = 2, nrow = 1,
  plot(unite_ex_data_3, node_size = n_items, node_label = name,
       node_color = n_items, title = "All data"),
  plot(subset(subsampled, n_items > 0, subtaxa = FALSE),
     node_size = n_items, node_color = n_items,
     node_label = n_items, title = "Sampled"))
```
...
plot(subsetted, node_size = n_items, node_label = name, 
     node_color = n_items, overlap_avoidance = 0.5, tree_label = name)
```
...
set.seed(3)
plot(sampled, node_size = n_items, node_label = name, 
     node_color = n_items, overlap_avoidance = 0.5, tree_label = name)
```
...
```{r, fig.height=2.5}
set.seed(3)
plot(pcr, node_size = n_items, node_label = name, 
...
```{r, fig.height=2.5}
subset(pcr, name == "Bacteria") %>%
  subset(count_amplified < n_items | count_amp_2 < n_items, subtaxa = F) %>%
  plot(node_size = n_items, node_label = name,
