...
# Creating a taxonomically balanced reference sequence set
...
* **Taxonomically-balanced subsample:** Functions `extract_taxonomy` and `taxonomic_sample` can be used to parse taxonomy information from a sequence file (e.g. from FASTA headers with Genbank ids) and the subsample/resample the data to make it taxonomically balanced.
...
The first step is to use the taxonomy tree structure stored in `taxa` (in the form of an adjacency/edge list) to subsample `items`.
Then, `taxa` is subsampled using `restrict_taxonomy` to remove any taxa not associated with items in the subset.
...
Similarily, species will be subsampled to 5 sequences each to avoid any one species introducing bias.
...
It might also be desireable to not included sequences from underrepresented taxa.
Too few sequences means that the diversity of the taxon cannot be determined and questionable sequences are not as obvious without others of the same classifiaction to compare them to.
...
We can now view the difference between the original and subsampled data sets using `plot_taxonomy` again.
...
Although, downloading a data set, parseing it with `extract_taxonomy`, and subsamping with `taxonomic_sample` is the best method for most analyses, it is also possible to download a taxonomically balanced random set of sequences from Genbank without downloading all of the sequences for the group of interest.
...
If it is pheasable to download all of the group of interest, it is likly best to use `extract_taxonomy` + `taxonomic_sample` instead of `ncbi_taxon_sample`.
