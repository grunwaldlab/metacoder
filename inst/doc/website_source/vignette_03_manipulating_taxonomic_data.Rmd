...
Note that observations assigned to removed subtaxa are reassigned to the closest parent taxa that passes the filter by default.
...
Like observations, children of removed taxa are reassigned to the closest supertaxon that passes the filter.
...
```{r}
filter_obs(unite_ex_data_3, grepl("^A",  sequence_id)) %>%
  plot(node_size = n_obs,
       node_color = n_obs,
       node_label = name)
```
...
```{r}
filter_obs(unite_ex_data_3, grepl("^A",  sequence_id), unobserved = FALSE) %>%
  plot(node_size = n_obs,
       node_color = n_obs,
       node_label = name)
```
...
The functions `select_taxa` and `select_obs` are little more that wrappers for `dplyr::select`.
The only thing they do differently is to enforce that the `taxon_ids`, `parent_ids`, and `obs_taxon_ids` columns are preserved:
