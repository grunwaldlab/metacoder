...
data <- mutate_taxa(data,
                    read_counts = vapply(items(data),
                                         function(x) sum(data$item_data$totab[x]), numeric(1)))
```
...
data %>%
  ...
  plot(title = "Plankton diversity in the sunlit ocean",
       title_size = 0.03,
       vertex_color_axis_label = "Number of reads (Abundance)",
       vertex_size_axis_label = "Number of species (OTUs)",
       vertex_size = item_counts,
       vertex_size_range = c(0.0012, NA),
       vertex_color = read_counts,
       vertex_color_range = c("grey", "#80cdc1", "#018571", "#dfc27d", "#a6611a"),
       vertex_color_trans = "log10 radius",
       vertex_label = ifelse(grepl(pattern = "^[a-zA-z\\-]{1,25}$", name) &
       ...
       vertex_label_size_range = c(0.001, NA),
...
data <- mutate_items(data, sur_counts = apply(data$item_data[, sur_columms], MARGIN = 1,
                                              function(x) sum(as.numeric(x))))
data <- mutate_items(data, dcm_counts = apply(data$item_data[, dcm_columms], MARGIN = 1,
                                              function(x) sum(as.numeric(x))))

data <- mutate_taxa(data, sur_taxon_counts = vapply(items(data),
                                                    function(x) sum(data$item_data$sur_counts[x]), numeric(1)))
data <- mutate_taxa(data, dcm_taxon_counts = vapply(items(data),
                                                    function(x) sum(data$item_data$dcm_counts[x]), numeric(1)))

data <- mutate_taxa(data, depth_prop_dcm = dcm_taxon_counts / read_counts)
total_dcm_reads <- sum(data$taxon_data$dcm_taxon_counts[taxon_ranks(data) == 1])
total_sur_reads <- sum(data$taxon_data$sur_taxon_counts[taxon_ranks(data) == 1])
...
data %>%
  ...
  plot(title = "Location of taxa in water column",
       title_size = 0.03,
       vertex_color_axis_label = "DCM only        Both       Surface only",
       vertex_size_axis_label = "Number of species (OTUs)",
       vertex_size = item_counts,
       vertex_size_range = c(0.0012, NA),
...
data %>%
  ...
  plot(title = "Location of taxa in water column",
       ...
       vertex_size = item_counts,
       vertex_size_range = c(0.0012, NA),
       vertex_color = depth_prop_diff,
       vertex_color_range = diverging_palette(),
       vertex_color_trans = "radius",
       vertex_color_interval = c(-2, 2),
...
data %>%
  ...
  plot(title = "Location of taxa in water column",
       ...
       vertex_size = item_counts,
       vertex_size_range = c(0.0012, NA),
       vertex_color = depth_diff_test_stat,
       vertex_color_range = diverging_palette(),
       vertex_color_trans = "area",
       vertex_color_interval = c(-1000, 1000),
       edge_color_interval = c(-1000, 1000),
#        vertex_label = ifelse(grepl(pattern = "^[a-zA-z\\-]{1,25}$", name) &
#                                ! Reduce(`|`, lapply(taxa_patterns_to_hide, grepl, x = name)),
#                              name, NA),
       vertex_label = depth_diff_test_stat,
       vertex_label_size = (item_counts / taxon_ranks) ^ 0.5,
       vertex_label_size_trans = "area",
       vertex_label_size_range = c(0.001, NA),
       vertex_label_max = 1000,
...
data <- mutate_taxa(data, mean_pid = vapply(items(data),
                                            function(x) mean(data$item_data$pid[x], na.rm = TRUE), numeric(1)))
# Percentage of OTUs with less than 90% idententiy
data <- mutate_taxa(data, percent_known = vapply(items(data),
                                                 function(x) sum(data$item_data$pid[x] >= 90, na.rm = TRUE) / length(x) * 100, numeric(1)))
...
data %>%
  ...
  plot(title = "Poportion of OTUs not well identified",
       title_size = 0.03,
       vertex_color_axis_label = "Percent of species (OTUs) identified",
       vertex_size_axis_label = "Number of species (OTUs)",
       vertex_size = item_counts,
       vertex_size_range = c(0.0012, NA),
       vertex_color = percent_known,
       vertex_color_range = c("red", "orange", "yellow", "green", "cyan"),
       vertex_color_trans = "radius",
...
data %>%
  ...
  filter_taxa(read_counts >= 100) %>%
  filter_taxa(name == "Metazoa", subtaxa = TRUE) %>%
  plot(vertex_color_axis_label = "Percent of OTUs identified",
