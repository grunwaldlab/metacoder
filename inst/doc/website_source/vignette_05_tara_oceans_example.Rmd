...
### Caluculate read depths

```{r}
sample_columns <- sample_data[["PANGAEA ACCESSION NUMBER"]]
data <- mutate_items(data,
                     total_counts = apply(data$item_data[, sample_columns], MARGIN = 1,
                                          function(x) sum(as.numeric(x))))
data <- mutate_taxa(data,
                    total_taxon_counts = vapply(items(data),
                                                function(x) sum(data$item_data$total_counts[x]), numeric(1)))
```
...
```{r}
seed = 15 #9, 10, 12 is good
set.seed(seed)
taxa_patterns_to_hide <- paste0("^", c("[X]+", "X\\+sp\\.", "NA", "root", "\\*", "sp\\.", "sp", "Uncultured"), "$")
...
data %>%
  ...
  plot(title = "Eukaryotic plankton diversity in the sunlit ocean",
       title_size = 0.03,
       vertex_color_axis_label = "Number of reads",
       vertex_size_axis_label = "Number of species (OTUs)",
       vertex_size = item_counts,
       vertex_size_range = c(0.0015, NA),
       vertex_color = total_taxon_counts,
       vertex_color_range = c("white", "#018571", "#80cdc1", "#dfc27d", "#a6611a", "#a6611a", "#a6611a"),
       ...
       vertex_label_size_range = c(0.0006, NA),
       tree_label = name,
       ...
       output_file = file.path(output_folder, paste0("tara_oceans--read_count_all_seed", seed,  output_format)))# 
```
...
filter_taxa(data, name == "Metazoa", subtaxa = TRUE) %>%
  ...
  plot(title_size = 0.03,
       ...
       vertex_label = name,
       vertex_color = mean_pid,
       vertex_color_range = c("darkred", "red", "orange", "yellow", "cyan", "blue"),
       vertex_color_trans = "radius",
       vertex_color_interval = c(60, 97),
       edge_color_interval = c(60, 97),
