...
unite_ex_data_1 <- extract_taxonomy(sequences[!grepl(pattern = "\\|UDB", names(sequences))],
                            regex = "^(.*)\\|(.*)\\|(.*)\\|.*\\|(.*)$",
                            key = c(name = "name",  sequence_id = "item_id",
...
This is typically not as efficient or accurate and using `"item_id"` or `"taxon_id"`, so `"class"` has no effect if these more reliable fields (or others) are present and a database is used. 
