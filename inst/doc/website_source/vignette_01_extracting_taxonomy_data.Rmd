...
head(obs_data(genbank_ex_data, "input"))
```
...
```{r, echo=FALSE, comment=NA}
head(obs_data(unite_ex_data_1, "input"))
```
...
```{r, echo=FALSE, comment=NA}
head(obs_data(its1_ex_data, "input"))
```
...
The first observation in the header is sometimes a GenBank accession number, but not always.
...
```{r, echo=FALSE, comment=NA}
head(obs_data(pr2_ex_data, "input"))
```
...
```{r, echo=FALSE, comment=NA}
head(obs_data(rdp_ex_data, "input"))
```
