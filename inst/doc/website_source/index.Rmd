...
Note the `taxon_funcs` part contains function names (e.g. `item_counts`) used to calculate   taxon statistics.
...
There is corresponding `item_data` function and `item_funcs` list, but they are not used in this case.
...
Unlike the `dplyr` function `filter`, users can choose preserve or remove the subtaxa, supertaxa, and associated sample data of the selected taxa.
For example, `filter_taxa` can be used to look at just the Archaea:

```{r}
set.seed(1)
plot(filter_taxa(data, name == "Archaea"), vertex_size = item_counts, 
...
Although items (information in `item_data`) are typically assinged to the tips of the taxonomy, they can also be assigned to any internal node.
When a tip taxon is removed by a filtering operation, the user has the option to reassign it a supertaxa that was not filtered out.
This makes it easy to remove lower ranks of a taxonomy: 

```{r}
set.seed(1)
filter_taxa(data, taxon_ranks < 5, subtaxa = FALSE) %>%
plot(vertex_size = item_counts, 
     vertex_label = name, vertex_color = item_counts)

```

There is also a `filter_items` function which can filter out items and the taxa they are assigned to.
...
sampled <- filter_taxa(sampled, item_counts > 0, subtaxa = FALSE) 
```
...
This can also be accomplished using the `dplyr` equivalents to `sample_n` and `sample_frac` by weighting the probability of sampling items by the inverse of the number of items in taxa they are assigned to:

```{r}
set.seed(4)
sample_n_items(subsetted, size = 1000, taxon_weight = 1 / item_counts, itemless = FALSE) %>%
  plot(vertex_size = item_counts, vertex_label = item_counts, overlap_avoidance = 0.5,
     vertex_color = item_counts)
```
...
set.seed(7) 
pcr %>%
  filter_taxa(name == "Bacteria") %>%
  filter_taxa(count_amplified < item_counts, subtaxa = FALSE) %>% 
...
pcr <- mutate_taxa(pcr, 
                   count_amplified_2 = taxon_data(pcr_2, col_subset = "count_amplified", drop = T),
                   prop_diff = prop_amplified - taxon_data(pcr_2, col_subset = "prop_amplified", drop = T))
```
