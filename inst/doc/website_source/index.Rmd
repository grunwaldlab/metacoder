...
```{r}
set.seed(1)
plot(filter_taxa(data, name == "Archaea"), node_size = n_obs, 
...
This can also be accomplished using the `dplyr` equivalents to `sample_n` and `sample_frac` by weighting the probability of sampling observations by the inverse of the number of observations in taxa they are assigned to:
...
plot(pcr, node_size = n_obs, node_label = name, node_color = prop_amplified,
     node_color_range =  c("red", "cyan"), node_color_trans = "linear", tree_label = name)
```
...
pcr %>%
  ...
  filter_taxa(count_amplified < n_obs, subtaxa = FALSE) %>% 
