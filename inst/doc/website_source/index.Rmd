...
Note the `taxon_funcs` part contains function names (e.g. `n_items`) used to calculate   taxon statistics.
...
```{r}
set.seed(1)
plot(filter_taxa(data, name == "Archaea"), node_size = n_items, 
...
```{r}
set.seed(1)
filter_taxa(data, taxon_levels < 5, subtaxa = FALSE) %>%
plot(node_size = n_items, 
     node_label = name, node_color = n_items)
...
sampled <- filter_taxa(sampled, n_items > 0, subtaxa = FALSE) 
```
...
```{r}
set.seed(4)
sample_n_items(subsetted, size = 1000, taxon_weight = 1 / n_items, itemless = FALSE) %>%
  plot(node_size = n_items, node_label = n_items, overlap_avoidance = 0.5,
     node_color = n_items)
```
...
pcr %>%
  ...
  filter_taxa(count_amplified < n_items, subtaxa = FALSE) %>% 
