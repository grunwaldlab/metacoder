...
```{r}
subset(genbank_ex_data, taxon_ranks > 5) %>%
...
This works because supertaxa of filtered taxa are not preserved by `subset` by default.
...
```{r}
subset(unite_ex_data_3, taxon_ranks > 1) %>%
  plot(vertex_size = item_counts,
       vertex_color = item_counts,
       vertex_label = name,
       tree_label = name)
subset(unite_ex_data_3, taxon_ranks > 3) %>%
  plot(vertex_size = item_counts,
       vertex_color = item_counts,
       vertex_label = name,
       tree_label = name)
