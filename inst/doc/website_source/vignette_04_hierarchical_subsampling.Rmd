...
plot(subsampled,
     node_size = n_items,
     node_color = n_items,
     node_label = ifelse(taxon_levels %in% c(7, 4), n_items, NA),
     edge_label = ifelse(taxon_levels == 4, name, NA))
```
...
```{r}
filter_taxa(subsampled, n_items > 0, subtaxa = FALSE) %>%
...
```{r}
sample_n_items(unite_ex_data_3, size = 100, taxon_weight = 1 / n_items)
```

These functions also support sampling with repacement: 

```{r}
sample_n_items(unite_ex_data_3, size = 10000, taxon_weight = 1 / n_items, replace = TRUE)
```
