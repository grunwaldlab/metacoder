...
data$name <- factor(data$name, levels = unique(data$name[order(data$item_counts, decreasing = TRUE)]), ordered = TRUE)
...
ggplot(data[!data$unite_rank  %in% c("Kingdom", "Species", "Genus"), ]) + 
  geom_bar(aes(x = name, y = item_counts), stat = "identity") +
...
ggplot(data[data$unite_rank == "Phylum", ]) + 
  geom_bar(aes(x = factor(1), y = item_counts, fill = name), stat = "identity") +
  coord_polar(theta = "y") +
  labs(x = "Taxon", y = "Abundance", title = "Abundance of taxa in a sample of the UNITE database") +
  theme_minimal() +  
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), 
...
Although there are many options that can be used to make highly customized graphs, `plot_taxonomy` only needs one argument to function: an object of type `classified`.
We can see the default appearance of the data used in the introduction using the code below:

```{r}
head(taxon_data(unite_ex_data_3))
plot(unite_ex_data_3)
```
...
The size of vertices and lines can be scaled to a number associated with each taxon using the `vertex_size` and `edge_size` parameters. 
...
The `vertex_color` argument works in a similar way to `vertex_size`.
...
The range of color used can be set using the `vertex_color_range` argument. 
...
Like `vertex_size`, the color of lines can be set independently of vertices, although the default behavior is for the lines to have the same color as the vertices. 
