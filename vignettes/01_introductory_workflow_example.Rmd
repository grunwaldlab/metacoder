...
`extract_taxonomy` returns an object of type `classfied`.
This object stores information assoicated with taxa as well as sequences.
...
All data is in "Stamenopiles", so lets remove the redundant "Eukaryota" to make the data simpler
...
```{r}
subsampled_data <- taxonomic_sample(data,
                                    max_counts = c("6" = 3, "7" = 1),
                                    min_counts = c("4" = 3))
```
...
```{r}
subsampled_data <- subset(subsampled_data, item_count > 0, itemless = FALSE) 
```
...
```{r}
subsampled_data <- primersearch(subsampled_data,
