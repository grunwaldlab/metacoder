...
# Creating a taxonomically balanced sequence set
...
The `max_counts = c("o" = 20, "s" = 5)` option is used to implement the sub-sampling limits for each taxonomic rank (encoded by single letters for this specific dataset). 
...
```{r}
fungi <- ncbi_taxon_sample(name = "fungi", target_level = "phylum", max_counts = c(phylum = 30), entrez_query = "18S[All Fields] AND 28S[All Fields]", min_length = 600, max_length = 10000)
taxonomy <- extract_taxonomy(fungi$gi_no, regex = "(.*)", key = "item_id")
taxa <- taxonomy$taxa # Rename parts of the result for later convenience
plot_taxonomy(taxa$taxon_id, taxa$parent_id,
              size = taxa$item_count,
              vertex_color = taxa$item_count,
