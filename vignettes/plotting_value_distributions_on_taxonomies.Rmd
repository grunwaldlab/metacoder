...
# Plotting value distributions on taxonomies

------------

## Introduction

Visualizing statistics associated with taxa can be difficult due to the heirarchical nature of the information.
Traditional graph types used to visualize the relationship between catagories (e.g. taxa) and quantities (e.g. abundance) such as bar charts, pie charts, and box plots, are fundamentally two-dimensional. 
...
To demonstrate this idea, lets display the same data using various graphing techniques and compare their effectivness.
...
taxa$rank <- factor(taxa$rank, levels = unique(taxa$rank),
...
For ranks with relativly few taxa, this is a satisfactory graphing technique, but it is ineffective once there is more than 20 or so taxa. 
It also is difficult to decern how subtaxa are distributed within each taxon. 
For example, if we only looked at the phylum or class level, we can easily see that Basidiomycota/Agaricomyctes are the most plentiful, but dont know if that is due to a single highly abundant species, or many moderatly abundant species.
...
These two lists effectivly consitute an adjacency/edge list, describing how taxa are connected in a phylogenetic tree. 
We can see the defualt appearance of the data used in the introduction using the code below.
...
By defualt, the size of each vertex is inversely proportional to its rank. 
This makes higher ranked taxa smaller, making them easier to see. 
The size of verticies and lines can be scaled to a number associated with each taxon using the `size` parameter. 
...
Note that it was not necessary to specify the absolute vertex size; the range of absolute vertex sizes is optimized for each graph so as to minimize overlap of verticies and maximize the ranges of sizes.
The argumnet `overlap_bias` is used to determin how much overlaps are avoided. 
Higher values mean more importance is given to avoiding overlaping verticies than to maximizing the ranges of sizes.
A high `overlap_bias` makes the connections between taxa more clear, but deminishes the visual effect of vertex size.
...
This argument take a list of colors in the form of names, hex color codes, or intergers. 
...
Unlike `size`, the color of lines can be set independently of verticies, although the defualt behavior is for the lines to have the same color as the verticies. 
To only color vertices, you can set the lines to be a constant color:
...
You can also set the color pallette used for the lines in the same way as you set it for the vertex using the argument `line_color_series`. 

#### Vertex labels 

Labels can be added to verticies using the `vertex_label` option:
...
The minimum size of labels that will be printed is controled by the `min_label_size` option:
...
This means that the text size will always be proportional to the graph size regardless of ow big the grpah is rendered; however, these special labels take more time to render, so causing too many to be printed drastically slow the rendering of the graph. 

#### Line labels 

Lines can be labeled as well using the `line_label` option, which works similarlly to the `vertex_label` option:
