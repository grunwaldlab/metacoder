...
library(metacoder)
library(knitr)
opts_chunk$set(dev='svg', cache = TRUE, warning = FALSE, message = FALSE)
```
...
`extract_taxonomy` returns two `data.frame`s:

* `taxa`: containes information for every unique taxon
* `items`: containes information for evey item (e.g. sequence), including the taxon id defined in `taxa`

Lets take a look at the first few rows of `taxa` to get and idea of its structure.

```{r, results='asis', echo=FALSE}
kable(head(taxa))
```

To easily see the results, we can use `plot_taxonomy` to see how the abundance of different taxa are distributed. 
...
index <- taxonomic_sample(root_id = "1", item_ids = items$taxon_id, taxon_ids = taxa$taxon_id,
                          parent_ids = taxa$parent_id, ranks = taxa$level, max_counts = c("7" = 3, "8" = 1),
                          min_counts = c("5" = 3))
...
We can then plot the results as we did before.
...
plot_taxonomy(sampled_taxa$taxon_id, sampled_taxa$parent_id,
              ...
              vertex_label = sampled_taxa$name)
```
...
`get_taxon_count` can be used to count how many sequences were amplified at every rank. 

```{r}
amplified_counts <- get_taxon_count(taxa$taxon_id, taxa$parent_id, amplified_taxa) 
```

We can then calculate the proportion of sequences for each taxon that was amplified. 

```{r}
proportion_amplified <- amplified_counts / taxa$item_count 
```

If there were multiple amplifications from a single sequence, the proportion could be above 1. 
We will cap the proportion at 1 to make the graphing easier to read. 

```{r}
proportion_amplified[proportion_amplified > 1] = 1 
```

Finally, lets see the results by making the vertex color correspond to the proportion of sequences amplified:
