...
genbank_tax <- extract_taxonomy(names(sequences)[1:3],
                                regex = "^.*\\|(.*)\\|.*\\|(.*)\\|(.*)$",
...
This is not as efficient or accurate and using "`item_id`" or "`taxon_id`", so `"class_name"` has no effect if these more reliable fields are present. 
However, you can still capture the sequence id or taxon id (assuming its present in the header) by using `"item_info"` or `"taxon_info"` instead where you would otherwise use "`item_id`" or "`taxon_id`".
...
The unique taxon id for each taxon encountered will be looked up and taxa not found will be given arbitrary ids, unless the `allow_arb_ids` option is set to `FALSE`, in which case an error will occure if taxa are not found.
...
cat(names(sequences)[1])
tax_info <- extract_taxonomy(names(sequences)[5:6],
                             regex = "^(.*\\..*)\\..*\\|(.*)$",
...
cat(names(sequences)[1])
tax_info <- extract_taxonomy(names(sequences)[212:214],
                             regex = "^(.*?) (.*) (.*)\\tLineage=(.*)",
