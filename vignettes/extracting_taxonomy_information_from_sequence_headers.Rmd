...
#### Using the included classifications

It is also possible to build a custom taxonomy encoding using the taxonomy string (e.g. `k__Fungi;p__Ascomycota;c__Leotiomycetes`) in the sequence headers.
...
tax_info <- extract_taxonomy(names(sequences)[1:5],
                             regex = "^(.*)\\|(.*)\\|(.*)\\|.*\\|(.*)$",
                             key = c(name = "item_info", item_id = "item_info",
