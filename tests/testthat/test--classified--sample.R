...
obj <- taxmap(taxon_ids = LETTERS[1:5], parent_ids = c(NA, 1, 2, 2, 1), 
                  ...
                  item_data = data.frame(item_attr = rep(LETTERS[1:10], 10),
                                         stringsAsFactors = FALSE))
original_plot <- plot(obj, node_label = paste(taxon_ids, n_items), node_color = n_items, layout = "fr")
#|
#| ####  Sampling items by taxon_weights
test_that("Sampling items by taxon_weights works", {
  result <- sample_n_items(obj, size = 10, taxon_weight = 1 / n_items, replace = FALSE, use_supertaxa = TRUE)
